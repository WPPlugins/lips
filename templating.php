 ...
 * 
 * $Id: templating.php 705272 2013-04-29 08:23:38Z bastb $
 * 
 */
...
class LinkedInProfileSyncSmarty extends Smarty {
	...
	public function __construct($variables = array(), $smarty_reporting = false) {
		...
		$this->setCompiledTemplatesDirectory();
	...
	/** 
	 * Creates a temporary directory for the compiled templates. The
	 * directory is created in the systems' temp directory, and is 
	 * produced using a static string and, optionally, a number. The 
	 * directory is created when it doesn't already exist
	 * 
	 */
	protected function setCompiledTemplatesDirectory() {
		$i = 0;
		$compile_dir_candidate = $compile_dir = sys_get_temp_dir() . '/wp_lips_compiled_tpl';
		$has_dir = false;
		
		do {
			// Here's where the trouble starts'
			if (file_exists($compile_dir_candidate)) {
				$has_dir = is_dir($compile_dir_candidate);
				if (! $has_dir) { 
					$i++;
					// Add the suffix and try again
					$compile_dir_candidate = $compile_dir . '_' . $i;
				}
			}
			else {
				// Create directory recursively
				@mkdir($compile_dir_candidate, 0755, true);
				// This keeps the loop running when the process cannot write
				// to the temporary directory
				$has_dir = is_dir($compile_dir_candidate);
			}
		} while (! $has_dir);
		
		// Set the compile directory to this newly created (or selected) directory
		$this->setCompileDir($compile_dir_candidate);
	}
	...
	public function fetch($data) {
		...
		// Template caching and compiling should be disabled, so there's
		// no need to save a compiled version
		$this->clearCompiledTemplate();
