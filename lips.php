...
Description: Synchronizes your professional LinkedIn profile, updating WordPress Pages and - optionally - Posts.
Version: 0.8.15
...
License: GPL2
 ...
 Copyright (C) 2012, 2013 Bas ten Berge
 ...
 $Id: lips.php 791451 2013-10-21 19:58:02Z bastb $
...
class LinkedInProfileSyncOptions {
	...
	// has an OAuth token already. Needed for the Save button to function correctly
	protected $jquery_oauth_initialized = false;
	...
	public function __construct() {
		...
		$this->sections['ext']  = __( 'More Services');
	...
	public function prepare() {
		...
		OAuthStore::instance("LipsLinkedIn", $oauth_options);
	}
...
EOS;
	...
	<div class="lips-help"><span class="lips-help-text">' . $this->getAboutText() . '</span><a href="javascript:void(0);" class="button lips-close-help" id="lips-close">Close</a></div>';
		...
		echo '<div id="lips-page-box"><p>' . __('LiPS uses a page to display the profile or debug-profile on. You can create a new page, which is recognized by the plugin. Enter a title for your new page in the textbox:') . '</p><input type="text" id="lips-page" name="' . SETTINGS_ID . '[lips-page]" class="regular-text" /><p>'.__('Click the Create button to create the page. The settings page will reload and any unsaved changes will get lost') .'</p></div>';
		echo '<div id="lips-err-box">' . '<p><span id="lips-err-text">' . __('There is a problem getting an authorization code from LinkedIn&reg;:') . '</p></span><span id="lips-err-detail" class="lips-err-detail"></span><p><span id="lips-err-additional-detail">' . __('This may be caused by incorrect OAuth Identification, a slow internet connection or a problem at LinkedIn&reg;. Retry in a couple of minutes if you did not change anything. Read the posts at ') . '<a href="http://developer.linkedin.com/forums/rest">' . __('LinkedIn REST forum') . '</a>' . __(' when the problem persists.') . '</p></span></div>';
		echo '<h3 class="nav-tab-wrapper">';

		foreach ( $this->sections as $section_slug => $section ) {
			$additional_classes = '';
			if ('li' == $section_slug) {
				$additional_classes = ' nav-tab-active';
			}
			echo '<a href="#' . $section_slug . '" class="nav-tab ' . $additional_classes . ' lips-tab-' . $section_slug . '">' . $section . '</a>';
		}
	...
	public function display_section($section) {
		$section_description['li'] = __("Use this section to configure basic settings. The plugin is ready for use when you completed the configuration in this section.");
		...
		$section_description['ext'] = __("You can include account details of other services, like") . '<a href="http://stackexchange.com/sites" target="lips-se-sites"> ' . __("the Stack Exchange sites") . '</a>' . __(", to the static page you're displaying your profile on.");
	...
	/**
	 * Adds the unselect option to the list of options, at the very first position
	 */
	protected function addUnselectOptionTo($option_value, $label, $remaining_options) {
		return array($option_value =>"< " . $label . " >") + $remaining_options;
	}
	...
	protected function addNoPageSelectedOptionTo($option_value, $remaining_options) {
		return $this->addUnselectOptionTo($option_value, "Select page ...", $remaining_options);
	}

	protected function addNoStackExchangeSelectedOptionTo($option_value, $remaining_options) {
		return $this->addUnselectOptionTo($option_value, "Select Stack Exchange Service ...", $remaining_options);
	}
	...
	protected function getOAuthSettings($available_pages) {
		...
		try {
			$oauth = LinkedInProfileSyncOAuth::fromTokenStore($this->tokenstore, false);
			$this->jquery_oauth_initialized = true;
			...
			// Add the Locally Installed Templates and Custom Templates options
			$this->settings['page_template'] = array(
				'title' => __('Page Content'),
				'desc' => __('Select the method to maintain page content'),
				'type' => 'radio',
				'section' => $section,
				'choices' => array(
					'page_use_installed_template' =>__("Select an installed page template"),
					'page_use_custom_template' => __("Create your own template"),
				),
				'std' => 'page_use_installed_template',
			);
			...
			foreach ($meta_manager as $value) {
				...
				$this->settings['statics'] = array(
					...
					'choices' => $rows,
					'depends' => 'statics_container page_use_installed_template',
					'class' => 'lips_static page_use_installed_template',
				);
			...
			$this->settings['installed_page_template'] = array(
				...
				'previous' => 'page_template',
				'required' => true,
				'has_meta' => true,
				'class'    => 'page_use_installed_template',
				'depends'  => 'page_use_installed_template',
			);
	...
	protected function getOtherServicesSettings() {
		$section = 'ext';
		$has_stored_settings = true;
		$sites = array();
		$can_do_stackexchange = true;
		$stacknet_desc = sprintf('%s <a href="http://www.stackexchange.com" class="lips-ext-ref">Stack Exchange Network</a> %s', __( 'Select the'), __('site which hosts your account'));

		try {
			$se = LinkedInProfileSyncStackExchangeUserDetails::fromMetadata();
		}
		catch (LipsMetadataNotFoundException $e) {
			$has_stored_settings = false;
		}
		catch (LipsRequirementMissingException $e) {
			$can_do_stackexchange = false;
			$stacknet_desc = sprintf("%s <strong>%s</strong>. %s", __('The platform the plugin is running on is missing'), $e->getMessage(), __('You cannot include your Stack Exchange account details until this issue is resolved'));
		}

		if ($can_do_stackexchange) {
			$networks = LinkedInProfileSyncStackExchangeSites::fromTransientData();
			$sites = $networks->fetch();
			$networks->save();
		}

		$key = 'stackexchange_site';
		$this->settings[$key] = array(
			'desc'   => $stacknet_desc,
			'title'    => __( 'Stack Exchange Network' ),
			'type'    => 'select',
			'choices' => $this->addNoStackExchangeSelectedOptionTo($this->jquery_service_type, $sites),
			'section' => $section,
			'value' => '',
		);

		if ($has_stored_settings && $can_do_stackexchange) {
			$this->settings[$key]['value'] = $se->getService();
		}

		if ($can_do_stackexchange) {
			$key = 'stackexchange_login';
			$this->settings[$key] = array(
				'desc'   => sprintf('%s <a href="http://www.stackexchange.com" class="lips-ext-ref">Stack Exchange Site</a> %s', __( 'Use your'), __('loginname. Enter the numeric id representing your account when your loginname is not that unique')),
				'title'    => __( 'Stack Site Account' ),
				'type'    => 'text',
				'std'     => '',
				'section' => $section,
				'value'   => '',
			...
			if ($has_stored_settings) {
				$this->settings[$key]['value'] = $se->getAccount();
			}
		}
	}
	...
	*/
	public function scripts() {
		wp_register_script('lips-script', plugins_url('js/lips.js', __FILE__), array('jquery', 'jquery-ui-tabs', 'jquery-ui-dialog'));
		wp_localize_script('lips-script', 'sections', array_flip($this->sections));
		wp_localize_script('lips-script', 'errors', $this->jquery_error_details);
		wp_localize_script('lips-script', 'autorun', $this->jquery_autorun);
		wp_localize_script('lips-script', 'statics', $this->jquery_static_message);
		wp_localize_script('lips-script', 'sample_links', $this->jquery_sample_page);
		wp_localize_script('lips-script', 'sample_link_text', __('Example'));
		wp_localize_script('lips-script', 'language_specific', $this->jquery_available_profiles);
		wp_localize_script('lips-script', 'page_usage', $this->jquery_page_usage_types);
		wp_localize_script('lips-script', 'no_page_selection', $this->jquery_no_page_selected);
		wp_localize_script('lips-script', 'no_service', $this->jquery_service_type);
		wp_localize_script('lips-script', 'oauth_initialized', $this->jquery_oauth_initialized ? "t" : "f");
		wp_localize_script('lips-script', 'dialog', array(
	...
	public function styles() {
		wp_register_style('lips-style', plugins_url('css/lips.css', __FILE__), array());
 		wp_enqueue_style('wp-jquery-ui-dialog');
 		wp_enqueue_style('jquery-ui-tabs');
		wp_enqueue_style('lips-style');
	}
	...
	*/
	public function validate_settings($input) {
		...
		if (! isset($validated["profile_source"])) {
			$validated["profile_source"] = 'li_profile';
		}

		$id = $this->tokenstore->getIdentificationToken();
		$do_profile_update = null != $id;
		...
		if (array_key_exists('pin', $input) && trim($input['pin']) != "") {
			...
			$oauth = LinkedInProfileSyncOAuth::fromTokenStore($this->tokenstore, false);
			if (! $oauth->authorize(trim($input["pin"]), array($this, "setOAuthError"), array("pin" => $input["pin"], "oauth" => $oauth, "class" => "auth"))) {
				$errcount++;
			}
		}
		...
		if (array_key_exists('stackexchange_site', $validated) && $this->jquery_service_type != $validated['stackexchange_site']) {
			if (array_key_exists('stackexchange_login', $validated)) {
				$user = trim($validated['stackexchange_login']);
				if (empty($user)) {
					unset($validated['stackexchange_login']);
				}
				else {
					$validated['stackexchange_login'] = $user;
					add_filter(LIPS_PROFILE_FETCHED_FILTER, array($this, 'addStackExchangeUserDetails'), 43);
				}
			}
		}
		...
		if ($do_profile_update && 0 == $errcount) {
			...
			$this->validated_settings = $validated;
	...
	public function performProfileUpdate() {
		$profile_source = $this->validated_settings["profile_source"];
		if ($profile_source == "li_profile") {
			$sync = new LinkedInProfileSyncer($this->validated_settings, $this->tokenstore);
		}
		else if ($profile_source == "local_profile") {
			$sync = new LinkedInProfileSyncer($this->validated_settings, null);
		}
		...
		if (null == $errors) {
			$profile_page_id = $this->validated_settings["profile_page"];
	...
	public function setOAuthError($callback_specific, $e) {
		...
		if (is_array($callback_specific)) {
			...
			add_settings_error($callback_specific, "oauth_authorization_1", $message . "<br/>" . $e->getMessage());
		}
	 ...
	 */
	protected function skipIgnoredNotification($message, $property_id, $wrap_in_link = true, $is_error = false) {
		...
		if (! is_array($ignore) || ! array_key_exists($property_id, $ignore) || ! $ignore[$property_id]) {
			return $this->createNotificationMessage($message, $property_id, $wrap_in_link, $is_error);
		}
		return "";
	}

	protected function createNotificationMessage($message, $property_id, $wrap_in_link = true, $is_error = false) {
		...
		$div->setAttribute('class', $notification_type . ' fade');
 ...
 */
	...
	public function generifyTemplateDataFilter($json, $wpobj) {
		...
		if (!array_key_exists('ext', $to_return['x_lips']) || !array_key_exists('providers', $to_return['x_lips'])) {
			$to_return['x_lips']['ext']['providers'] = array();
		}
	...
	public function keepCopyOfProfilePictureFilter($json) {
		...
		// Don't download the profile picture when it is stored to WordPress already.
		...
		if ($download_picture && array_key_exists($key, $json) && !empty($to_return[$key])) {
			...
			foreach ($json[$key]['values'] as $picture_url) {
				...
				if (empty($previous_dimensions) || $dimensions != $previous_dimensions) {
					...
					$saved = @file_put_contents($path, file_get_contents($picture_url));
					// The user may not have permission to store the file
					if (FALSE !== $saved) {
						$picture_meta = array(
	...
	public function saveFormattedDebugPostFilter($json) {
		$to_return = $json;
		$page_filter = array(
			'include' => array($this->validated_settings['profile_debug_data_page']),
			'post_status' => array_keys(get_page_statuses()),
		);
		...
		if ($this->validated_settings['profile_debug_use_geshi']) {
	...
	/**
	 * Adds user details of the users' reputation on stackoverflow.com
	 */
	public function addStackExchangeUserDetails($json) {
		$uid = $this->validated_settings['stackexchange_login'];
		$service = $this->validated_settings['stackexchange_site'];
		$is_saved = true;

		try {
			$se = LinkedInProfileSyncStackExchangeUserDetails::fromMetadata();
			$is_saved = $se->isSame($uid, $service);
		}
		catch (LipsMetadataNotFoundException $e) {
			$is_saved = false;
		}
		...
		if (!$is_saved) {
			$se = new LinkedInProfileSyncStackExchangeUserDetails($uid, null, $service);
		}

		$decoded = $se->fetch();
		$to_return = $json;
		if (array_key_exists('items', $decoded) && 1 == count($decoded['items'])) {
			$to_return['x_lips']['ext']['stackexchange']['account'] = $decoded['items'][0];
			$to_return['x_lips']['ext']['stackexchange']['service'] = $service;
			if (!array_key_exists('provider', $to_return['x_lips']['ext'])) {
				$to_return['x_lips']['ext']['providers'] = array('stackexchange');
			}
			else {
				$to_return['x_lips']['ext']['providers'] += array('stackexchange');
			}
			$stacknet = new LinkedInProfileSyncStackExchangeSites();
			$stacksite = $stacknet->fetchFiltered('api_site_parameter', $service);
			$to_return['x_lips']['ext']['stackexchange']['site'] = $stacksite;
		}
